% topoplotg() - plot a topographic map of a scalp data field in a 2-D circular view 
...
%              plot gradient as well (current directions).
% Usage:
%        >>  topoplotg(datavector, EEG.chanlocs);   % use a channel locations structure
%        >>  topoplotg(datavector, 'my_chan.locs'); % read a channel locations file
%        >>  topoplotg('example'); % Gives an example of an electrode location file
...
if ~strcmpi(STYLE,'blank') % if draw interpolated scalp map
  ...
  %
  %%%%%%%%%%%%%%%%%% Plot gradient (current) directions  (Alon Keren 12.2.09) %%%%%%%%%%%%%
  %
  v=2:4:66;
  [px,py] = gradient(-Zi(v,v));
  quiver(Xi(1,v),Yi(v,1),px,py,'k')
  %
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% Set color axis  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  %
