...
% 1. Clear the BIOSIG plugin path, as it is failing its own check.
        rmpath(genpath(fileparts(which('eeglab.m'))));
        
        % 2. Re-add only the necessary path:
        addpath(fileparts(which('eeglab.m')));
        
        % 3. Use the working function again, forcing the load this time.
        % We trust the environment is clean now.
