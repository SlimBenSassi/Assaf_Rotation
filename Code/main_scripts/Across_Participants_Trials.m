...
for sub_idx = 1:N_SUBJECTS
    ...
    [latencies, codes, intensities, obj_outcomes, subj_outcomes, conditions, isis, n_trials] = select_single_trials_behavior(SDATA, warning_codes_of_interest, all_warning_codes, subject_id_str);
    ...
    [latencies, codes, intensities, obj_outcomes, subj_outcomes, conditions,isis, n_trials] = reject_artifact_trials(SDATA, latencies, codes, intensities, obj_outcomes, subj_outcomes, conditions, isis, total_epoch_samples, pre_samples);
...
column_names = {'ObjectiveOutcome', 'SubjectiveOutcome', 'Condition', 'IrregularTargetTime', 'AlphaAmplitude', 'StimIntensity', 'SubjectID'};


% --- Now the table creation is simplified and robust ---
MasterTable = table(all_objective_outcome, all_subjective_outcome, all_conditions, all_isis, all_alpha_power_raw, all_stim_intensity_raw, all_subject_ids, 'VariableNames', column_names); % <-- Uses clean variable
