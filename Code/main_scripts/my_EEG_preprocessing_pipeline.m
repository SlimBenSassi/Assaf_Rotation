...
% Temporary Bypass: HARDCODE LOCAL PATH to test loading function
raw_data_path = 'C:\Users\ssassi\Desktop\Assaf_Rotation\Data\Assaf Data (controls)'; % The folder where you copied the BDF
data_file_list = {'RIC_s303.bdf'}; % The file you copied
...
disp('reading data files with EEGLAB/BIOSIG')
tic
for sessIdx = 1:nSessions
    
    fileName = fullfile(raw_data_path,data_file_list{sessIdx});
    
    % *** 1. Use the robust EEGLAB function to load the data structure ***
    [EEG_temp] = pop_biosig(fileName);
    
    % *** 2. Map data/header info from the temporary EEG_temp structure ***
    % Extract the data and transpose it (as your original script does)
    session_data = single(EEG_temp.data)'; 
    
    % Get the header information needed later in your script
    hdr.Fs = EEG_temp.srate;
    hdr.label = {EEG_temp.chanlocs.labels}; % Assuming channel locations were loaded
    hdr.nSamples = EEG_temp.pnts;
    hdr.nChans = EEG_temp.nbchan;
...
%TODO check why data dimension changes to 71 and to 70 even when i press no 
